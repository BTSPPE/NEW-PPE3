      ...
      <div class="modal-header">
        ...
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            ...
            foreach($offreVoulu as $uneOffre){
              echo "<div class='flex'>";
              echo  "<div id='offreVoulu' idoffre=".$uneOffre->idOffre." class='boiteDeals'>".$uneOffre->nomService .'<br>'. $uneOffre->descriptionOffre.'<br>'. $uneOffre->dateOffre.'</div><br><br>';
            }
            echo "</div>";
            if(count($maDemande)!=0){
              ...
              foreach($maDemande as $laDemande){
                echo "<div id='demandeVoulu' iddemande=".$laDemande->idDemande." class='boiteDeals'>".$laDemande->nomService.'<br>'.$laDemande->descriptionDemande.'<br>'.$laDemande->dateDemande."</div><br>";
              }
              echo "</div>";
            }
            else{
            ...
            if($i==true){
              if(count($listeMesOffres)!=0 && count($listeSesDemandes)!=0){
                ...
                foreach($listeMesOffres as $uneChose){
                  echo "<div offreUser1=".$uneChose->idOffre." class='boiteDeals mesOffres'>".$uneChose->nomService.'<br>'.$uneChose->descriptionOffre.'<br>'.$uneChose->dateOffre."</div><br><br>";
                }
                echo "</div>";
                echo "Ses demandes actuelles <br>";
                echo "<div class='flex'>";
                foreach($listeSesDemandes as $unTruc){
                  echo "<div iddemande2=".$unTruc->idDemande." class='boiteDeals sesDemandes' >".$unTruc->nomService.'<br>'.$unTruc->descriptionDemande.'<br>'.$unTruc->dateDemande.'</div><br><br>';
                }
                echo "</div>";
              }
              else{
